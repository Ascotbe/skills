---
name: changesets
description: Create or refresh a `.changeset/(slug).md` for the current branch, or
  report that none is required. Triggers on "/changesets create", "add a changeset",
  "create a changeset", "update the changeset", "refresh the changeset", "do I need
  a changeset", or any work that touches `packages/**` source files on a feature branch.
---
# /changesets create
