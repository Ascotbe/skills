...
description: 'Remediate GitHub Actions action findings identified by the action-audit
  skill. Applies the appropriate fix per action type — `@main` ref for internal `bitwarden/`
  actions, full SHA with inline version comment for external actions, or full replacement
  — across selected repos and creates draft PRs. Run the action-audit skill first
  to identify findings before using this skill. (example) User: Go ahead and fix the
  unpinned actions from the audit Action: Trigger action-remediate to apply fixes
  and create PRs (/example) (example) User: Replace tj-actions/changed-files with
  the safe version across those repos Action: Trigger action-remediate to swap the
  action and create PRs (/example)'
---
## Rules
